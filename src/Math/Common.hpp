...
namespace Math {
    ...
    static inline constexpr auto modulo(const PrecisionInX x, const PrecisionInM m) {
        if (x < static_cast<PrecisionInX>(0))
            return static_cast<PrecisionOut>(static_cast<int>(x * oneOver<PrecisionOut>(m) - 1) * m - x);
        else
            return static_cast<PrecisionOut>(x - static_cast<int>(x * oneOver<PrecisionOut>(m)) * m);
    }
    template <typename PrecisionOut, typename PrecisionInX> static inline constexpr PrecisionOut modTau(const PrecisionInX x) {
        if (x < static_cast<PrecisionInX>(0))
            return static_cast<PrecisionOut>(static_cast<int>(x * oneOver<PrecisionOut>(TAU) - 1) * TAU - x);
        else
            return static_cast<PrecisionOut>(x - static_cast<int>(x * oneOver<PrecisionOut>(TAU)) * TAU);
    }
    template <typename PrecisionOut, typename PrecisionInX> static inline constexpr PrecisionOut modPi(const PrecisionInX x) {
        if (x < static_cast<PrecisionInX>(0))
            return static_cast<PrecisionOut>(static_cast<int>(x * oneOver<PrecisionOut>(PI) - 1) * PI - x);
        else
            return static_cast<PrecisionOut>(x - static_cast<int>(x * oneOver<PrecisionOut>(PI)) * PI);
    }
    template <typename PrecisionOut, typename PrecisionInX>
    static inline constexpr PrecisionOut modHalfPi(const PrecisionInX x) {
        if (x < static_cast<PrecisionInX>(0))
            return static_cast<PrecisionOut>(static_cast<int>(x * oneOver<PrecisionOut>(HALF_PI) - 1) * HALF_PI - x);
        else
            return static_cast<PrecisionOut>(x - static_cast<int>(x * oneOver<PrecisionOut>(HALF_PI)) * HALF_PI);
    }
} // namespace Math
