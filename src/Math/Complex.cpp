...
namespace Coel {
	namespace Math {
		...
		////////////////////////////////////
		template <typename T>
		inline Complex<T, 2> &Complex<T, 2>::operator+=(const Complex<T, 2> &c)
		{
			r += c.r;
			i += c.i;
			return *this;
		...
		inline Complex<T, 2> Complex<T, 2>::operator/(const Complex<T, 2> &c)
		{
			...
			return {r * c.r / rmagSQ + i * c.i / rmagSQ,
				i * c.r / rmagSQ - r * c.i / rmagSQ};
		}
		template <typename T>
		T &Complex<T, 2>::operator[](int index) const
		{
		...
		////////////////////////////////////
		template <typename T>
		inline Complex<T, 4> &Complex<T, 4>::operator+=(const Complex<T, 4> &c)
		{
			r += c.r;
			i += c.i;
			j += c.j;
		...
		{
			return {r * c.r - i * c.i - j * c.j - k * c.k,
				r * c.i + i * c.r + j * c.k - k * c.j,
				r * c.j - i * c.k + j * c.r + k * c.i,
		...
		template <typename T>
		T &Complex<T, 4>::operator[](int index) const
		{
