...
namespace math {
    ...
    template <typename T> struct Complex<T, 2> {
        ...
        inline constexpr auto operator*(const Complex<T, 4> &c) const {
            return Complex<T, 2>{r * c.r - i * c.i, r * c.i + i * c.r};
        }
        inline constexpr auto operator/(const Complex<T, 4> &c) const {
            const T inverse_magnitude_sq = static_cast<T>(1) / (sq(c.r) + sq(c.i));
            return Complex<T, 2>{r * c.r * inverse_magnitude_sq + i * c.i * inverse_magnitude_sq,
                                 i * c.r * inverse_magnitude_sq - r * c.i * inverse_magnitude_sq};
        }
        inline constexpr auto operator*(const T a) const { return Complex<T, 4>{r * a, i * a}; }
        inline constexpr auto operator/(const T a) const { return Complex<T, 4>{r / a, i / a}; }
        ...
        inline constexpr auto length() const { return static_cast<T>(INTERNAL_MATH_SQUARE_ROOT_FUNC(sq(r) + sq(i))); }
    ...
    template <typename T> struct Complex<T, 4> {
        ...
        inline constexpr auto operator*(const Complex<T, 4> &c) const {
            return Complex<T, 4>{r * c.r - i * c.i - j * c.j - k * c.k, r * c.i + i * c.r + j * c.k - k * c.j,
        ...
        inline constexpr auto operator/(const Complex<T, 4> &c) const {
            ...
            return Complex<T, 4>{r * c.r * inverse_magnitude_sq + i * c.i * inverse_magnitude_sq +
                                     j * c.j * inverse_magnitude_sq + k * c.k * inverse_magnitude_sq,
                                 i * c.r * inverse_magnitude_sq - j * c.k * inverse_magnitude_sq +
                                     k * c.j * inverse_magnitude_sq - r * c.i * inverse_magnitude_sq,
                                 i * c.k * inverse_magnitude_sq + j * c.r * inverse_magnitude_sq -
                                     k * c.i * inverse_magnitude_sq - r * c.j * inverse_magnitude_sq,
                                 j * c.i * inverse_magnitude_sq + k * c.r * inverse_magnitude_sq -
                                     r * c.k * inverse_magnitude_sq - i * c.j * inverse_magnitude_sq};
        }
        inline constexpr auto operator*(const T a) const { return Complex<T, 4>{r * a, i * a, j * a, k * a}; }
        inline constexpr auto operator/(const T a) const { return Complex<T, 4>{r / a, i / a, j / a, k / a}; }
        inline constexpr auto &operator+=(const Complex<T, 4> &c) {
        ...
        inline constexpr auto &operator/=(const Complex<T, 4> &c) {
            ...
            k = temp_j * c.i * inverse_magnitude_sq + k * c.r * inverse_magnitude_sq - temp_r * c.k * inverse_magnitude_sq -
        ...
        inline constexpr auto length() const {
            return static_cast<T>(INTERNAL_MATH_SQUARE_ROOT_FUNC(sq(r) + sq(i) + sq(j) + sq(k)));
        }
        inline constexpr auto &normalize() {
            T factor = static_cast<T>(1) / length();
