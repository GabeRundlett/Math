...
namespace math {
    ...
    template <typename T> constexpr Vector<T, 2> Vector<T, 2>::operator+(const Vector<T, 2> &v) const {
        return {x + v.x, y + v.y};
    }
    template <typename T> constexpr Vector<T, 2> Vector<T, 2>::operator-(const Vector<T, 2> &v) const {
        return {x - v.x, y - v.y};
    ...
    template <typename T> constexpr Vector<T, 3> Vector<T, 3>::operator*(const T fac) const {
